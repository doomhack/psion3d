#ifndef FP_MATH_H
#define FP_MATH_H

#include "fp_types.h"

#define TRIG_TABLE_LEN 1024
#define TRIG_TABLE_MASK (TRIG_TABLE_LEN - 1)
#define TRIG_COS_OFFSET (TRIG_TABLE_LEN / 4)
#define TRIG_RAD_TO_INDEX 163
#define TRIG_RAD_TO_INDEX_SHIFT 8

extern const f16 sincos_tab[TRIG_TABLE_LEN];

static s16 trigidx(const f16 rad)
{
	/* (rad * TRIG_RAD_TO_INDEX) >> 8 is exactly fpmul's operation, so routing
	   through it picks up the assembler multiply as well. Relies on
	   TRIG_RAD_TO_INDEX_SHIFT being FP_BITS. */
	return (s16)fpmul(rad, TRIG_RAD_TO_INDEX);
}

static f16 fpsin(const f16 rad)
{
	return sincos_tab[trigidx(rad) & TRIG_TABLE_MASK];
}

static f16 fpcos(const f16 rad)
{
	return sincos_tab[(trigidx(rad) + TRIG_COS_OFFSET) & TRIG_TABLE_MASK];
}

#endif